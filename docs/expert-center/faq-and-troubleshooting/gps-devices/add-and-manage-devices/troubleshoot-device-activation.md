...
If a device is new or has not been used for an extended period of time
...
A device is installed in the wrong direction
...
Incorrect device ID or phone number is specified on the platform
...
A device cannot send valid GPS coordinates
...
Device's timezone is not UTC+0
...
A custom password for SMS commands is set on a device
...
Mistakes in manually sent commands
...
A device may attempt to transfer data from its buffer
...
Firmware or device modification is not supported
...
A device is located in an area with weak GSM signal
...
A device is located in an area with a weak mobile internet signal
...
SIM card does not have SMS feature available
...
SIM card does not have the mobile Internet data service activated
...
SIM card has run out of internet traffic
...
SIM card is damaged or not installed correctly
...
Sending data to our server IP is blocked by the ISP
...
The communication standard on a device does not match the communication standard on a SIM card
...
A panel does not have an SMS gateway
...
SMS gateway does not support sending spaces at the beginning of a message
...
The "Do not send default settings" option is enabled in the panel settings
...
Insufficient funds in the user account where you activate the device
...
Insufficient funds in your SMS gateway account
...
Your SMS gateway cannot deliver message to a device
