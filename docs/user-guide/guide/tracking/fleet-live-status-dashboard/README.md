...
The Dashboard app has a second tab, the **Trips Dashboard**, which focuses on completed trips over the last 7 days rather than the live state of your fleet. For the Trips tab, see [Trips dashboard](trips-dashboard.md).
...
* **Check who's at your points of interest:** the Points of Interest panel lists POIs that currently have one or more objects inside their radius, useful when your operations are organised around specific stops rather than larger zones.
...
The **Points of Interest** panel is the companion to Geofences for accounts that organise operations around specific stops rather than larger zones. While a geofence is an area you draw on the map, a point of interest is a single location with a radius around it: a customer site, a pickup point, a fuel station, a checkpoint. The panel lists POIs that currently have at least one of your objects inside that radius, along with the count and the labels of those objects.

The table has three sortable columns:

* **Poi Label:** the name of the point of interest, as configured in your account.
* **Object Count:** how many of your objects are currently within the POI's radius.
* **Objects Label List:** the labels of those objects, listed alphabetically. Objects that don't have a label set appear as `No Data`.

Rows are sorted by object count from highest to lowest by default, so the busiest POIs surface at the top. Like Geofences, this panel is scoped to where activity is happening right now: POIs with no objects nearby are not listed, and the panel shows up to 100 POIs at a time, ranked by object count. If your account has more POIs than that with activity, the lower-ranked ones aren't shown until traffic shifts.

If the panel is empty, it means no objects are currently inside any POI radius for your account. The panel updates within a few minutes of an object entering or leaving a POI, the same cadence as the Geofences panel. POI labels are shown as configured; the panel does not display coordinates or resolved addresses.
