...
**Enable / disable**, plus **thresholds** for acceleration, braking, and (usually) cornering. The **unit and tuning style differ by device family**:
...
* **Speed-range tables** (different thresholds at low/medium/high speed) and **vehicle presets** (car / bus / truck) — e.g. some Teltonika/Queclink.
...
Appears on devices that support harsh-event detection (not all models do; some report only a subset of event types).
